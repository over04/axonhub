...
func TestInboundPersistentStream_Close_IncompleteStillPersistsChunks(t *testing.T) {
	...
	// The local fork's request-content isolation gates LoadResponseChunks on an
	// owner user context; load the persisted chunks as the system owner.
	ownerCtx := contexts.WithUser(ctx, &ent.User{IsOwner: true})
	chunks, err := requestService.LoadResponseChunks(ownerCtx, dbReq)
	require.NoError(t, err)
